...
SSH経由でリモートの Nextcloud サーバーに対して `occ` コマンドを実行・管理するための CLI ツールです。
...
- **直感的なサブコマンド体系**: Nextcloud の各種 `occ` コマンドを用途別に整理。
...
# 未スキャン (変更があった) ファイルのみを高速スキャン (--unscanned)
cargo run -- file-scan --all --unscanned

# 各種オプションの組み合わせ (ロックなし、ホーム領域のみ、再帰なし)
cargo run -- file-scan --all --unscanned --shallow --home-only --no-lock
```

| オプション | 説明 |
| :--- | :--- |
| `--all` | 全ユーザーのファイルをスキャンします。 |
| `-p, --path <PATH>` | 指定したパスのみをスキャンします。 |
| `[USER_ID]` | 指定したユーザーのみをスキャンします。 |
| `--unscanned` | サイズ未計算・未スキャンの差分ファイルのみを高速スキャンします。 |
| `--shallow` | サブディレクトリを再帰走査せず、指定階層直下のみスキャンします。 |
| `--home-only` | 外部ストレージや共有を除外し、ホームストレージ領域のみスキャンします。 |
| `--no-lock` | トランザクションファイルロックを行わずにスキャンを実行します。 |
...
本プロジェクトは [MIT License](LICENSE-MIT) および [Apache License 2.0](LICENSE-APACHE) のデュアルライセンスです。
